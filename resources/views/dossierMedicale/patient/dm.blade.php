...
<body style="background:rgb(246, 246, 246); padding-right: 0px !important;">
...
<div class="col-md-12" style="background: #3F4079; background: -webkit-linear-gradient(to left, #827FFE, #3F4079);background: linear-gradient(to left, #827FFE, #3F4079);" >
                    ...
                    <div style="width: 100%;" ><h2 style="color:#fff;width: 50%; margin: 0 auto 0 auto;text-align: center;">Panneau de Controle</h2></div>
            </br></br>
</div>

<div class="d-flex justify-content-center mt-2"><a href="/medcine/homeMS" class="col-md-4 btn btn-outline-success">Go Back to Dashboard</a></div>
    
<div class="container col-md-10">
<div class="card" style="margin-top:25px;">
  <h5 class="card-header" style="background:white; color: #3F4079"><i class="fas fa-folder-open"></i> : {{$objet}}</h5>
  <div class="card-body">
    </div>
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    tr.append("<td data-label='ID'><input value='" + data[i].id + "' disabled></td>");
...
<!-- axios conseil -->
    <script>
        ...
        downloadButton.addEventListener("click",function () {
            ...
            let response = axios({ method: 'post', url: 'blobmp3', data: form });
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                for (var i = 0; i < data.length; i++) {
                    tr = $('<tr class="trccc">');
                    data[i].type ==  0 ? X = Y: X = Z;
                    tr.append("<td><input value='" + data[i].id + "' disabled></td>");
                    data[i].type ==  0 ? tr.append("<td><select class='ty' ><option value='" + 0 + "'>" + Y + "</option><option value='"+ 1 + "'>" + Z + "</option></select></td>") : tr.append("<td><select class='ty' ><option value='" + 1 + "'>" + Z + "</option><option value='"+ 0 + "'>" + Y + "</option></select></td>");
                    tr.append("<td><input type='date' value='" + data[i].dateControleConsultation + "' class='t'></td>");
